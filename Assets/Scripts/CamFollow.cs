...
namespace Prototype.NetworkLobby
{
    public class CamFollow : NetworkBehaviour
    {
        ...
        void Start()
        {
            camPosition = new Vector2(GameObject.Find("Camera").transform.position.x, GameObject.Find("Camera").transform.position.y);
        ...
        void Update()
        {
            if (LobbyTopPanel.IsInGame())
            {
                if (player != null)
                {
                    ...
                    Vector2 camPosition = new Vector2(GameObject.Find("Camera").transform.position.x, GameObject.Find("Camera").transform.position.y);
